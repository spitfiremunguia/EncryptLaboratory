...
namespace EncryptLab
{
    public class Encrypt
    {
        ...
        public static string FFunction(string RightHalf, string Key)
        {
            ...
            List<int[,]> S_Matrices = new List<int[,]>();
            #region Adds SBoxMatrices
            S_Matrices.Add(new int[,]{
            ...
            S_Matrices.Add(new int[,]{
                ...
                {13,8,10,1,3,15,47,2,11,6,7,12,0,5,14,9}
            });
            S_Matrices.Add(new int[,]{
            ...
            S_Matrices.Add(new int[,]{
                {4,11,2,114,15,0,8,13,3,12,9,7,5,10,6,1},
            ...
            {
                addresses.Add(E.Substring(i, 6)); // we have 8 groups, 8 bits each one
            }
            StringBuilder output = new StringBuilder();
            for(int i = 0; i < addresses.Count; i++)
            {
                output.Append(SBoxFunction(addresses[i], S_Matrices[i]).PadLeft(4,'0'));
            }
            
            return Utilities.permutate(P_Matrix, output.ToString());
        }

        public static string SBoxFunction(string s, int[,] S_Matrix) {
        ...
        private static string EncodeData(string M,bool o)
        {
            int[,] Ip = new int[,]{
